...
thresholdImage<-function(filesList,channel,side){
  ...
  for(i in 1:length(filesList)){
    print(paste0("loading ",channel, " channel...", i, "/", length(filesList)," ", filesList[i]))
    currentImage<-filesList[i]
    c1[,,i]<-pixmap::getChannels(readTiff(currentImage), colors = channel)
  }
  #this is all commented out because the thresholding from R is not as good as the one from matlab, so I will now feed thresholded images to script
...
NormRawPipeline<-function(redImg,greenImg,xysize,xydimSearch,sampleSize,xyRealDim, pipeBinSize, timePoint, position){
  ...
  finalData$timePoint<-timePoint
  finalData$position<-position
  
  return(finalData)
}
GetBiomass<-function(redImg,greenImg,xysize,xydimSearch, timePoint, position){
  voxCoordsRed<-CorrectSampleArea(IdVoxel(redImg),xysize,xydimSearch)
  voxCoordsGreen<-CorrectSampleArea(IdVoxel(greenImg),xysize,xydimSearch)
  biomass<-data.frame(nrow(voxCoordsRed),nrow(voxCoordsGreen), timePoint, position)
  colnames(biomass)<-c("Red", "Green", "Time Point", "position")
  return(biomass)
}