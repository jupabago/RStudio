...
## Test Area
```{r test Area, echo=FALSE, fig.width=4, fig.height=3, results='hide', warning=FALSE}
...
RawBinning<-function(dfDistances){#takes the list of pairwise distances between focus and object in box and outputs frequency density table binned by distances of 2 uM
  numDistances<-apply(dfDistances,1,as.numeric)#turn input df into numeric so hist() works
  bins<-seq(0, max(dfDistances)+2,2)#establish the binning
  histogram<-hist(numDistances, breaks = bins, plot = FALSE)
  histCounts<-as.data.frame(histogram$counts)
  histDensity<-as.data.frame(histogram$counts/length(numDistances))
  histDensityFunction<-as.data.frame(histogram$density*2)
  return(cbind(histogram$mids, histCounts, histDensity, histDensityFunction))
  #return(numDistances)
}

testBinning1<-Binning(testDistanceBox1)
testRawBinning1<-RawBinning(testDistanceBox1)
