...
testBullseye <- array(0, c(540, 720, 9))
testBullseye[,,1]<-readTiff("ppbullseye.tiff")@red



testImageCoords <- IdVoxel(testBullseye)

c1 <- array(0, c(540, 720, 10))
c1[,,1]<-pixmap::getChannels(readTiff("bullseye.tiff"), colors = "red")
c1_t <- array(0, c(800, 800, 9))
th <- autoThreshold(c1[,,1], mean(c1[,,1]))[2]
c1_t[1][c1[1] > th] <- 1 
